...
namespace Microsoft.EntityFrameworkCore.Dm.Query.Internal
{
	public class DmMathTranslator : IMethodCallTranslator
	{
		...
		public virtual SqlExpression Translate(SqlExpression instance, MethodInfo method, IReadOnlyList<SqlExpression> arguments, IDiagnosticsLogger<DbLoggerCategory.Query> logger)
		{
			...
			{
				RelationalTypeMapping typeMapping = ((arguments.Count == 1) ? ExpressionExtensions.InferTypeMapping(new SqlExpression[] { arguments[0] }) : ExpressionExtensions.InferTypeMapping(new SqlExpression[]
				{
					arguments[0],
					arguments[1]
				}));
				SqlExpression[] array = new SqlExpression[arguments.Count];
				array[0] = _sqlExpressionFactory.ApplyTypeMapping(arguments[0], typeMapping);
				if (arguments.Count == 2)
				{
					array[1] = _sqlExpressionFactory.ApplyTypeMapping(arguments[1], typeMapping);
				}
				return _sqlExpressionFactory.Function(value, (IEnumerable<SqlExpression>)array, true, array.Select((SqlExpression a) => true).ToArray(), method.ReturnType, (value == "SIGN") ? null : typeMapping);
			}
			if (_truncateMethodInfos.Contains(method))
			{
				SqlExpression argument = arguments[0];
				return _sqlExpressionFactory.Function("TRUNCATE", new SqlExpression[]
				{
					argument,
					_sqlExpressionFactory.Constant(0, null),
					_sqlExpressionFactory.Constant(1, null)
				}, true, new bool[3] { true, false, false }, method.ReturnType, argument.TypeMapping);
			}
			if (_roundMethodInfos.Contains(method))
			{
				SqlExpression roundArg = arguments[0];
				SqlExpression digitsArg = (SqlExpression)((arguments.Count == 2) ? ((object)arguments[1]) : ((object)_sqlExpressionFactory.Constant(0, null)));
				return _sqlExpressionFactory.Function("ROUND", new SqlExpression[] { roundArg, digitsArg }, true, new bool[2] { true, true }, method.ReturnType, roundArg.TypeMapping);
			}
			return null;
		}
	}
